...
**Что изменено и добавлено по сравнению с первой версией:**

`RULES_TEXT` встроен в константы вверху файла и используется в двух местах — в уведомлении о новой песне (автоматически прикрепляется к каждому уведомлению) и в отдельной команде `/rules`.

**Новая команда `/rules`** — любой модератор может вызвать правила в чате в любой момент.

**`RULES_TEXT` как строка-конкатенация** — без тройных кавычек, чтобы избежать проблем с heredoc и спецсимволами при копировании.

**Для Render нужен файл `requirements.txt`:**
```
